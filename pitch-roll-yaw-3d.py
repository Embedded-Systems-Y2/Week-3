...
(line_yaw,)   = ax1.plot([], [], label="Yaw (°)", color='green')
...
seat_patch = Poly3DCollection([[seat[j] for j in [0,1,2,3]]], facecolor='green', alpha=0.7, edgecolor='black')
...
backrest_patch = Poly3DCollection([[backrest[j] for j in [0,1,2,3]]], facecolor='blue', alpha=0.7, edgecolor='black')
...
# Floor grid
ax2.plot([-2,2],[0,0],[0,0], color='gray', linestyle='--')
ax2.plot([0,0],[-2,2],[0,0], color='gray', linestyle='--')
...
def rotation_matrix(pitch, roll, yaw):
    ...
    Rz = np.array([[np.cos(yaw_rad), -np.sin(yaw_rad),0],
                   ...
                   [0,0,1]])
    return Rz @ Ry @ Rx  # yaw → roll → pitch
...
def init():
    ...
    line_roll.set_data([], [])
    line_yaw.set_data([], [])
    return line_pitch, line_roll, line_yaw, seat_patch, backrest_patch, marker, legs
...
def update(frame):
    ...
    if pitch_buf and roll_buf and yaw_buf:
        ...
        # Rotate legs
        rotated_legs = [np.array([p,q]) @ R.T for p,q in leg_coords]
