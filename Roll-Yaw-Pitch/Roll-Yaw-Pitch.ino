...
float yaw   = 0.0f;   // Z axis (gyro-integrated)
...
void setup() {
  ...
  roll = acc_roll;
  pitch = acc_pitch;
  yaw = 0.0f; // no absolute ref for yaw
...
void loop() {
  ...
  float yaw_gyro   = yaw   + gz * dt;  // yaw is only gyro-based

  // Complementary filter (roll/pitch corrected by accel)
  roll  = alpha * roll_gyro  + (1.0f - alpha) * acc_roll;
  pitch = alpha * pitch_gyro + (1.0f - alpha) * acc_pitch;
  yaw   = yaw_gyro; // no accel correction → expect drift
